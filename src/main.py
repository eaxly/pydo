...
def noice(msg_string: str, msg_type: str) -> str:
    ...
    if msg_type == "":  # if forgot to precise the msg_type
        console.print(
            f"[bold](??) UNKNOWN: The developer forgot to precise what type of information this is. Please submit an issue to fix this.\nMESSAGE:\n[/bold]{msg_string}")
...
def check_task(task_id: str, task_list_todo: dict, task_list_done: dict, tasks_to_write: dict):
    ...
    remove_task(task_id=task_id, task_list=task_list_todo,
                tasks_to_write=tasks_to_write, write_file=False)
...
def uncheck_task(task_id: str, task_list_todo: dict, task_list_done: dict, tasks_to_write: dict):
    ...
    remove_task(task_id=task_id, task_list=task_list_done,
                tasks_to_write=tasks_to_write, write_file=False)
...
def arg_parse(args: list):
    if "--help" in args:
        ...
        check_task(args[1], task_list_todo=tasks_todo,
